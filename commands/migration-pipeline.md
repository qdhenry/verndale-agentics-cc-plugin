---
name: verndale:migration-pipeline
description: "Run the end-to-end Salesforce migration analysis pipeline — takes a client video recording and produces gap analysis, validation report, client elicitation, and Confluence export"
...
```bash
skills/extract-video-frames/scripts/extract-frames.sh "{video-path}" {interval} "{FRAMES_DIR}"
```
...
Prompt: |
  ...
  python skills/dedupe-frames/scripts/dedupe-frames.py "{FRAMES_DIR}" --keep-originals
...
Prompt: |
  ...
  Run:
  uv run skills/elevenlabs-transcribe/scripts/transcribe.py "{FRAMES_DIR}/full_audio.aac" --output "{FRAMES_DIR}/audio-transcript.txt"

  If the command fails, try with the original video file:
  uv run skills/elevenlabs-transcribe/scripts/transcribe.py "{video-path}" --output "{FRAMES_DIR}/audio-transcript.txt"
...
Prompt: |
  ...
  Follow the frame-analyst skill in skills/frame-analyst/SKILL.md.
...
Prompt: |
  Follow the frame-synthesis skill in skills/frame-synthesis/SKILL.md.
...
Prompt: |
  Follow the meeting-analyst skill in skills/meeting-analyst/SKILL.md.
...
Prompt: |
  Follow the ui-migration skill in skills/ui-migration/SKILL.md.
...
Prompt: |
  Follow the integration-api skill in skills/integration-api/SKILL.md.
...
Prompt: |
  Follow the data-schema skill in skills/data-schema/SKILL.md.
...
Prompt: |
  Follow the sfcc-b2b-expert skill in skills/sfcc-b2b-expert/SKILL.md.
...
Prompt: |
  Follow the gap-analysis skill in skills/gap-analysis/SKILL.md.
...
Prompt: |
  Follow the sfcc-validator skill in skills/sfcc-validator/SKILL.md.
